...
def build_scene(scene):
    ...
    for texMap in scene.texMaps:
        ...
        if texMap.type == MapType.diffuse:
            mat.use_nodes = True
            tree = mat.node_tree
            diffuse_shader = tree.nodes.get('Principled BSDF')
            if diffuse_shader:
                texture_node = tree.nodes.new('ShaderNodeTexImage')
                texture_node.image = tex.image
                links = tree.links
                link = links.new(texture_node.outputs[0], diffuse_shader.inputs['Base Color'])
    ...
    for meshData in scene.meshes:
        ...
        if len(tverts):
            ...
            uv_layer.data.foreach_set('uv', [coord for i in tfaceIndices for coord in tverts[i]])
    ...
    for skinData in scene.skins:
        ...
        for i in range(len(scene.meshes)):
            if scene.meshes[i].nodeIndex == skinData.nodeIndex:
                mesh = meshes[i]
        mesh_world_matrix = mesh.matrix_world
        bounding_box_min = mathutils.Vector(mesh.bound_box[0])
        bounding_box_max = mathutils.Vector(mesh.bound_box[6])
        bounding_box_length = (mesh_world_matrix @ bounding_box_max - mesh_world_matrix @ bounding_box_min).length
        defaultBoneLength = bounding_box_length / 40 / scene.scale
...
def load(operator, context, filePath, scale, diffuseOnly):
    ...
    # Set the viewport shading mode to Material Preview, to render textures
